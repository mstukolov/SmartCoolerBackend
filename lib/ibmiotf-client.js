...
const notificationController = require(path.join(__dirname, '..', '/server/controllers/notificationController')) ;
...
function eventDeviceDefinition(_previousValue, json){
    ...
    var maxDelta = 1.5
    var currValue = json['d']['param1'];
    _vmax = json['d']['param2'];

    //MAKS 25/10/2017 add event for alert about change bottle require
    //31/10/2017 Потребовалось убрать, т.к. много сообщений, сделал периодический опрос по таймеру в модуле storeProceduresController(periodicChangeBottleRequireNotification)
    /*if(currValue/_vmax < 0.01){
        notificationController.sendChangeBottleRequireNotification(json['d']['deviceid'], currValue)
    }*/
    if(  delta > 0){
